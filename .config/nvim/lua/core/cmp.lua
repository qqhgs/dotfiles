  ...
  xvim.builtin.cmp = {
    ...
    formatting = {
      ...
      format = function(entry, vim_item)
        vim_item.kind = xvim.builtin.cmp.formatting.kind_icons[vim_item.kind]
        vim_item.menu = xvim.builtin.cmp.formatting.source_names[entry.source.name]
        vim_item.dup = xvim.builtin.cmp.formatting.duplicates[entry.source.name]
          or xvim.builtin.cmp.formatting.duplicates_default
        return vim_item
      end,
    },
    snippet = {
    ...
    mapping = {
      ...
      ["<CR>"] = cmp.mapping(function(fallback)
        if cmp.visible() and cmp.confirm(xvim.builtin.cmp.confirm_opts) then
          if jumpable() then
            luasnip.jump(1)
          end
          return
...
function M.setup()
  require("cmp").setup(xvim.builtin.cmp)
end
